...
int quantity(int max, int min, double e)
{
    double result = log(2 / e);
    result *= sqrt(max / min) * 0.5;
    return (int)result + 1;
}
...
int main()
{
    ...
    int max = 100;
    int min = 1;
    int p = quantity(max, min, epsilon); //количество итераций
    ...
    while (ratio >= epsilon) {
        count++;
        double t = max + min - (max - min) * cos((2 * count - 1) * pi / 2 * p);
        t = 2 / t;
